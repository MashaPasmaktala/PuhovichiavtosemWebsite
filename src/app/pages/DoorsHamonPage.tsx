...
import { ArrowLeft, ArrowRight, X, ShoppingCart, Phone, Shield, Zap, Sparkles, Flame, Droplet, Check } from 'lucide-react';
...
// Размеры - только по запросу менеджера
const HAMON_SIZES: string[] = [
  'Уточняйте у менеджера'
];

// Цвета для прозрачных дверей Хамон
const CLEAR_COLORS: ColorOption[] = [
  { 
    ...
    type: 'clear',
    colorCode: 'linear-gradient(145deg, #F0F0F0, #E0E0E0)',
    textColor: '#000000'
  },
  { 
    id: 'hamon-clear-bronze', 
    name: 'Бронза прозрачная', 
    image: '/images/hamon-clear-bronze.jpg', 
    type: 'clear',
...
// Цвета для матовых дверей Хамон (включая сатин)
const MATT_COLORS: ColorOption[] = [
  { 
    id: 'hamon-matt-bronze', 
    name: 'Бронза матовая', 
    image: '/images/hamon-matt-bronze.jpg', 
    type: 'matt',
  ...
  { 
    id: 'hamon-satin', 
    name: 'Сатин', 
    image: '/images/hamon-satin.jpg', 
    type: 'matt',
    colorCode: 'linear-gradient(145deg, #E0E0E0, #C0C0C0)',
    textColor: '#000000'
  }
];
...
const products: Product[] = [
  {
    id: 1,
    name: 'Дверь для хамам прозрачная',
    type: 'clear',
    baseImage: '/images/hamon-clear.jpg',
    colors: CLEAR_COLORS,
    description: 'Прозрачная термостойкая дверь для хамама. Максимальная видимость и стиль.',
    sizes: HAMON_SIZES,
    features: [
      'Термостойкое стекло 6-8 мм',
      'Алюминиевая коробка',
      'Магнитный замок',
    ...
    specs: {
      glassThickness: '6-8 мм',
      frameMaterial: 'Алюминий',
      maxTemp: '300°C',
      lock: 'Магнитный замок',
      hinges: '3 петли в цвет профиля',
      handle: 'Алюминиевая ручка',
    }
  },
  {
    id: 2,
    name: 'Дверь для хамам матовая',
    type: 'matt',
    baseImage: '/images/hamon-matt.jpg',
    colors: MATT_COLORS,
    description: 'Матовая и сатиновая термостойкая дверь для хамам. Приватность и элегантность.',
    sizes: HAMON_SIZES,
    features: [
      'Матовое/сатиновое стекло 6-8 мм',
      'Алюминиевая коробка',
      'Магнитный замок',
      '3 петли в цвет профиля',
      'Алюминиевая ручка',
...
export function DoorsHamonPage() {
  ...
  const [isNonStandard] = useState(false);
  ...
  const getMinPrice = (product: Product) => {
    return product.type === 'matt' ? 1000 : 900;
  };
      ...
      <div className="w-screen">
        <div className="w-full px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 lg:py-12">
          ...
          <div className="text-center mb-8 sm:mb-10 md:mb-12 lg:mb-16 relative z-10">
            <div className="inline-flex items-center justify-center gap-1.5 sm:gap-2 md:gap-3 px-3 sm:px-4 md:px-5 py-1.5 sm:py-2 md:py-3 mb-3 sm:mb-4 md:mb-6 bg-gradient-to-r from-[#b50202]/30 to-[#8b0000]/30 border border-[#b50202]/40 rounded-full backdrop-blur-sm">
              <Flame className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-[#b50202] animate-pulse" />
              <span className="text-xs sm:text-sm md:text-base font-bold text-[#b50202] uppercase tracking-wider whitespace-nowrap">
                Термостойкое стекло 300°C
              </span>
            </div>
            ...
            <div className="text-sm sm:text-base md:text-lg lg:text-xl text-slate-400 max-w-3xl mx-auto px-2 sm:px-4 font-medium">
              Специальное термостойкое стекло для хамам. Выдерживает экстремальные температуры до 300°C и высокую влажность.
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 md:gap-6 mb-10 sm:mb-12 md:mb-16 lg:mb-20 relative z-10">
            {[
              {
                title: "Термостойкость 300°C",
                description: "Специальное закаленное стекло для экстремальных температур",
                icon: Flame
              },
              {
                title: "Алюминиевая коробка",
                description: "Легкий и прочный профиль, устойчивый к влажности",
                icon: Shield
              },
              {
                title: "Магнитный замок",
                description: "Надежная фиксация и герметичность",
                icon: Zap
              }
            ].map((feature, index) => {
                ...
                <div key={index} className="relative group fade-in-scroll">
                  ...
                  <div className="relative bg-gradient-to-b from-[#1a0000] to-[#0a0000] border-2 border-[#300000] rounded-xl md:rounded-2xl p-3 sm:p-4 md:p-6 hover:border-[#b50202]/50 transition-all duration-300 h-full">
                    <div className="flex flex-col items-center text-center gap-2 sm:gap-3 md:gap-4">
                      <div className="p-1.5 sm:p-2 md:p-3 bg-gradient-to-br from-[#b50202]/30 to-[#8b0000]/30 rounded-lg sm:rounded-xl border border-[#b50202]/40">
              ...
              <div key={product.id} className="group relative flex flex-col">
                ...
                <div className="relative bg-gradient-to-b from-[#1a0000] to-[#0a0000] border-2 border-[#300000] rounded-2xl sm:rounded-3xl overflow-hidden hover:border-[#b50202]/50 transition-all duration-500 group-hover:scale-[1.02] sm:group-hover:scale-[1.03] h-full flex flex-col">
                  ...
                  >
                    ...
                    {/* Temperature Badge */}
                    <div className="absolute top-3 right-3 sm:top-4 sm:right-4 px-2 sm:px-3 py-0.5 sm:py-1 bg-[#b50202]/90 backdrop-blur-sm rounded-full flex items-center gap-1 border border-[#b50202]/40">
                      <Flame className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" />
                      <span className="text-[10px] sm:text-xs font-bold text-white">300°C</span>
                    </div>
                  ...
                  >
                    ...
                    <div className="mt-auto">
                      ...
                      >
                        ...
                        <span className="relative whitespace-nowrap">Подробнее</span>
                      </button>
                    </div>
                  </div>
            ...
            <div className="relative bg-gradient-to-br from-[#1a0000] via-[#0a0000] to-[#1a0000] border-2 border-[#300000] rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 lg:p-12 text-center overflow-hidden group">
              ...
              <div className="relative">
                <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-black text-[#b50202] mb-3 sm:mb-4 md:mb-6 drop-shadow-[0_0_10px_rgba(181,2,2,0.5)]">
                  НУЖНА КОНСУЛЬТАЦИЯ ПО ДВЕРЯМ ДЛЯ ХАМАМ?
                </h2>
                <div className="text-xs sm:text-sm md:text-base lg:text-xl text-slate-300 mb-4 sm:mb-5 md:mb-6 lg:mb-8 max-w-2xl mx-auto px-2">
                  Наши специалисты помогут подобрать оптимальное решение для вашего хамама
                </div>
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 md:gap-4 justify-center px-2">
                  ...
                  >
                    ...
                    <span className="relative whitespace-nowrap">+375 (24) 669 44 44</span>
                  </a>
                  <Link
                    to="/contacts"
                  ...
                  >
                    <span className="whitespace-nowrap">Помощь с заказом</span>
            ...
            <div className="p-4 sm:p-6 md:p-8">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
                  ...
                  <div className="bg-[#300000]/20 rounded-lg sm:rounded-xl p-3 sm:p-4">
                    ...
                    <div className="space-y-1.5 sm:space-y-2">
                      ...
                      <div className="flex justify-between text-xs sm:text-sm">
                        ...
                        <span className="font-semibold text-white">{selectedProduct.specs.glassThickness}</span>
                      </div>
                      <div className="flex justify-between text-xs sm:text-sm">
                        <span className="text-slate-400">Материал коробки:</span>
                        <span className="font-semibold text-white">{selectedProduct.specs.frameMaterial}</span>
                      </div>
                      <div className="flex justify-between text-xs sm:text-sm">
                        <span className="text-slate-400">Макс. температура:</span>
                        <span className="font-semibold text-[#b50202]">{selectedProduct.specs.maxTemp}</span>
                      </div>
                      <div className="flex justify-between text-xs sm:text-sm">
                        <span className="text-slate-400">Замок:</span>
                        <span className="font-semibold text-white">{selectedProduct.specs.lock}</span>
                      </div>
                      <div className="flex justify-between text-xs sm:text-sm">
                        <span className="text-slate-400">Петли:</span>
                        <span className="font-semibold text-white">{selectedProduct.specs.hinges}</span>
                      </div>
                      <div className="flex justify-between text-xs sm:text-sm">
                        <span className="text-slate-400">Ручка:</span>
                        <span className="font-semibold text-white">{selectedProduct.specs.handle}</span>
                      </div>
                      {selectedColor && (
                        <div className="flex justify-between text-xs sm:text-sm">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-xs sm:text-sm md:text-base text-slate-400 mb-4 sm:mb-6 leading-relaxed">
                    {selectedProduct.description}
                  </p>

                  {/* Размеры */}
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-[#300000]/30 rounded-lg sm:rounded-xl border border-[#b50202]/20">
                    <h3 className="font-bold text-white mb-2 text-sm sm:text-base">Размеры:</h3>
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 bg-[#b50202] rounded-full animate-pulse"></div>
                      <p className="text-sm sm:text-base text-slate-300 font-semibold">
                        {selectedProduct.sizes[0]}
                      </p>
                    </div>
                    <p className="text-[10px] sm:text-xs text-slate-400 mt-2">
                      * Индивидуальный расчет размеров под ваш проем
                    </p>
                  </div>
